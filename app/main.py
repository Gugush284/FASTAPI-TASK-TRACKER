...
def delete_user(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(database.get_db)):
    user = auth.authenticate_user(db, form_data.username, form_data.password)
    if not user:
...
# Создание задачи
@app.post("/task/create", response_model=schemas.TaskOut, status_code=status.HTTP_201_CREATED)
...
def delete_task(task_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    if project_id:
        if db.query(models.Task).filter(models.Task.project_id == project_id).count() == 0:
...
@app.get("/projects/{project_id}/select_tasks", response_model=List[schemas.ProjectAlgOut])
def select_tasks_greedy(
    project_id: int,
    time_limit: int = Query(..., gt=0, description="Максимальное доступное время в минутах"),
